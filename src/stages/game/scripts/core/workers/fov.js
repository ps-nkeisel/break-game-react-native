...
var window = {};
    ...
    class Fov {
        ...
        setPolygonEnabledStatus = (fovID, enabled=true)=>{
            let polygon = this.scene.polygons.find((polygon)=>(
                polygon.fovID === fovID
            ));
            if (!polygon)
                return false;
        ...
        updateObserversAndUpdate = (arr, playerPos)=>{
            this.removeAllObservers();
            arr.forEach(item => {
                this.addObserver(item.x,item.y,item.lookAt, item.extraData, item.coneTint, item.coneAlpha);
            });
            this.setTargetPosition(playerPos.x, playerPos.y)
            return {updated:this.update(), observers: this.getObservers()};
        }
